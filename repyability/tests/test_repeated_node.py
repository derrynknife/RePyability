...
def test_repated_once_sf_correct_series():
    ...
    assert pytest.approx(model.sf(10)) == repeated_with_1.sf(10)
    assert pytest.approx(model.ff(10)) == repeated_with_1.ff(10)
...
def test_repated_twice_sf_correct_series():
    ...
    assert pytest.approx(model.sf(10) ** 2) == repeated_twice.sf(10)
    assert pytest.approx(1 - ((1 - model.ff(10)) ** 2)) == repeated_twice.ff(
        10
    )
...
def test_repated_50_sf_correct_series():
    ...
    assert pytest.approx(model.sf(10) ** 50) == repeated_twice.sf(10)
    assert pytest.approx(1 - ((1 - model.ff(10)) ** 50)) == repeated_twice.ff(
        10
    )
...
def test_repated_twice_sf_correct_parallel():
    ...
    assert pytest.approx(1 - ((1 - model.sf(10)) ** 2)) == repeated_twice.sf(
        10
    )
    assert pytest.approx(model.ff(10) ** 2) == repeated_twice.ff(10)


def test_repated_50_sf_correct_parallel():
    model = Weibull.from_params([10, 2])
    repeated_twice = RepeatedNode(model, 50, "parallel")
    assert pytest.approx(1 - ((1 - model.sf(10)) ** 50)) == repeated_twice.sf(
        10
    )
    assert pytest.approx(model.ff(10) ** 50) == repeated_twice.ff(10)
