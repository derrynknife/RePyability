...
# def availability_old(self, T, N=10_000) -> tuple:
#     """Returns the ???"""

#     # Aggregate timeline which maps times to system condition changes
#     # 0=unchanged, +1=activated, -1=deactivated
#     agg_timeline: dict[float, int] = defaultdict(lambda: 0)

#     # Perform N simulations
#     for i in range(N):
#         # PriorityQueue to keep Events ordered in time
#         pq: PriorityQueue[Event] = PriorityQueue()

#         # TODO
#         component_status = {}

#         # Set system condition at t=0 to working (1)
#         agg_timeline[0] += 1
#         system_status = 1

#         # Source random events for all components
#         for comp in self.reliability.keys():
#             component_status[comp] = 0  # TODO Why??

#             # Take random event for failure from each node.
#             t_initial = self.reliability[comp].random(1).item()

#             # Only add to priority queue if necessary
#             # i.e. if the event will happen in the simulation window
#             if t_initial < T:
#                 node = Event(t_initial, comp, 0)
#                 pq.put(node)
...
#                 # new_system_status = self.working(component_status)
...
#                 component_status[event.node] = event.status
...
#     tl = tl[tl[:, 0].argsort()]  # Sort by event time
#     tl[:, 1] = tl[:, 1].cumsum()  # Why take the cumsum()?
#     tl[:, 1] = tl[:, 1] / N  # Why divide by N?
